...
export default function SearchFilter({
  ...
  onFilter,
  placeholder = "Search...",
  /** When true (packages list), day/night uses item.packageCategory only. When false (bookings), day/night requires selectedPackage. */
  filterByItemPackageCategory = false,
  ...props
}) {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((item) => {
      ...
      const packageCategory =
        item?.selectedPackage?.packageCategory || item?.packageCategory;
      const hasPackage = item?.selectedPackage != null;
      let matchesCategory = true;
      if (activeTab === "unassigned") {
        matchesCategory = !hasPackage;
      } else if (activeTab === "day") {
        const isDay =
          packageCategory === "day_time" || packageCategory === "day";
        matchesCategory = filterByItemPackageCategory
          ? isDay
          : hasPackage && isDay;
      ...
      if (!isApiSearching && searchValueLower && filterKeys.length > 0) {
        matchesSearch = filterKeys.some((key) => {
          ...
          return (
            value && String(value).toLowerCase().includes(searchValueLower)
          );
        });
      }
  ...
  }, [data, activeTab, searchValue, filterKeys, filterByItemPackageCategory]);
