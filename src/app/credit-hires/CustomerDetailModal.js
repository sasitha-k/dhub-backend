...
import { User, Phone, Mail, MapPin, Calendar, Clock, DollarSign, Package, CheckCircle2 } from 'lucide-react';
...
export function CustomerDetailModal({ isOpen, onClose, customer, onSettlementSuccess }) {
  const { fetchBookingHistory, bookingHistory, isLoading } = useCustomerBookingHistory();
  ...
  const handleSettlementConfirm = async () => {
    ...
    try {
      ...
      toast.error(error?.response?.data?.message || error?.response?.data?.msg || "Failed to settle credit");
    } finally {
      setIsSettling(false);
    }
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="font-medium text-sm truncate">{customer.email || "N/A"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="font-medium text-sm truncate">{customer.address || "N/A"}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                ...
                <CardContent className="pt-0">
                  <div className="flex flex-col sm:flex-row gap-3 items-end">
                    <div className="flex-1 w-full">
                      <label className="text-xs text-muted-foreground mb-1 block">Settlement Amount</label>
                      <TextInput
                        type="number"
                  ...
                  <p className="text-xs text-muted-foreground mt-2">
                    Outstanding balance: <span className="font-semibold text-primary">{formatCurrency(customer.customerOwesCompany)}</span>
                  </p>
                </CardContent>
              </Card>
              <Separator />
              ...
              {isLoading ? (
                <div className="flex items-center justify-center h-32">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-muted-foreground text-sm">Loading booking history...</p>
                  </div>
                </div>
              ) : bookingHistory.length > 0 ? (
                    ...
                    <TableBody>
                      {bookingHistory.map((booking, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell>
                            <StatusBadge>{booking.status || "N/A"}</StatusBadge>
                          </TableCell>
                          ...
                          <TableCell>
                            {(() => {
                              const credit = parseFloat(booking.cashAmount || 0) - parseFloat(booking.fee || 0);
                          ...
                          <TableCell>
                            {booking.tripStartAt ? formatDateTime(booking.tripStartAt) : "N/A"}
                          </TableCell>
                          <TableCell>
                            {booking.tripEndAt ? formatDateTime(booking.tripEndAt) : "N/A"}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to settle <span className="font-semibold text-primary">{formatCurrency(settlementAmount)}</span> for{' '}
              <span className="font-semibold text-primary">
                {customer.firstName} {customer.lastName}
              </span>?
              <br />
              <br />
              Outstanding balance: <span className="font-semibold">{formatCurrency(customer.customerOwesCompany)}</span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel 
